...
-----
  ...
  * realized P/L per contract = (actual fill - exit) * 100.
...
def cmd_list():
    ...
    if not openish:
        ...
        return
    print(f"{'STATUS':8} {'ID':46} {'MODELED$':>9} {'FILL$':>7}")
    print("-" * 74)
    for r in openish:
        print(f"{r.get('status',''):8} {r.get('id',''):46} "
              f"{_fmt(r.get('modeled_credit_per_share')):>9} {_fmt(r.get('actual_fill_credit')):>7}")
...
def cmd_report():
    ...
    # 3) Realized edge / P&L
    pls = [r["realized_pl_per_contract"] for r in closed if r.get("realized_pl_per_contract") is not None]
    if pls:
        print("\n[3] Realized P/L per contract")
        print(f"    total  : {sum(pls):+.2f}")
        print(f"    average: {statistics.mean(pls):+.2f}")
        print(f"    win/loss avg: "
              f"{statistics.mean([p for p in pls if p>0] or [0]):+.2f} / "
              f"{statistics.mean([p for p in pls if p<=0] or [0]):+.2f}")
    print("=" * 60)
